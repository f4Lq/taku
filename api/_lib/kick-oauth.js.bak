...
const DEFAULT_KICK_OAUTH_REDIRECT_URI = "https://taku-live.pl/api/kick/oauth/callback";
const DEFAULT_KICK_OAUTH_ALLOWED_HOSTS = ["taku-live.pl", "www.taku-live.pl"];
...
function getKickOAuthAllowedHosts() {
  const raw = readEnv("KICK_OAUTH_ALLOWED_HOSTS");
  if (!raw) {
    return [...DEFAULT_KICK_OAUTH_ALLOWED_HOSTS];
  }
  const list = raw
    .split(",")
    .map((item) => normalizeHost(item))
    .filter(Boolean);
  return list.length ? Array.from(new Set(list)) : [...DEFAULT_KICK_OAUTH_ALLOWED_HOSTS];
}

function isKickOAuthAllowedHost(hostname) {
  const normalized = normalizeHost(hostname);
  if (!normalized) {
    return false;
  }
  const allowed = new Set(getKickOAuthAllowedHosts());
...
function resolveKickOAuthRedirectUri(requestUrl) {
  ...
  return DEFAULT_KICK_OAUTH_REDIRECT_URI;
}
...
function extractKickSubscribersCount(channel) {
  const total = readCountCandidates(channel, [
    // Prefer fields that usually represent total/current channel subscriptions.
    ...
    // Nested variants seen in some Kick payloads.
    ["data", "count"],
    ["data", "sub_count"],
    ...
    // Keep these as late fallback (often active/currently paid only).
    ["subscribers_count"],
    ["subscribersCount"],
    ["subscriber_count"],
    ["subscriberCount"],
...
module.exports = {
  ...
  DEFAULT_KICK_OAUTH_ALLOWED_HOSTS,
  DEFAULT_KICK_OAUTH_REDIRECT_URI,
