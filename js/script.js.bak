(() => {
  ...
  const LOCAL_DISCORD_AVATAR_CACHE_ENDPOINT = "/api/discord/avatar-cache";
  ...
  function decodeYouTubePathSegment(value) {
  ...
  function normalizeYouTubeHandle(value) {
    const clean = decodeYouTubePathSegment(String(value || "").trim()).trim();
    if (!clean) {
      return "";
    }
  ...
  function parseYouTubeChannelReference(rawValue) {
    ...
    const segments = String(parsed.pathname || "/")
      .split("/")
      .map((segment) => decodeYouTubePathSegment(segment).trim())
      .filter(Boolean);
  ...
  function renderAdminAccountsTable() {
    ...
    if (!accounts.length) {
      adminAccountsTableBodyEl.innerHTML = `
        <tr>
          <td colspan="12" class="admin-table-empty">Brak kont administracyjnych.</td>
        </tr>
      `;
      return;
    }
    ...
    accounts.forEach((account) => {
      ...
      const discordAvatarUrl = normalizeDiscordAvatarUrl(account.discordAvatarUrl);
      ...
      row.innerHTML = `
        <td>
          ${
            discordAvatarUrl
              ? `
                <a class="admin-account-avatar-link" href="${escapeHtmlText(discordAvatarUrl)}" target="_blank" rel="noopener noreferrer" aria-label="Otwórz avatar Discord">
                  <img class="admin-account-avatar" src="${escapeHtmlText(discordAvatarUrl)}" alt="${escapeHtmlText(discordName || account.login || "Avatar Discord")}" width="30" height="30" loading="lazy" decoding="async">
                </a>
              `
              : `<span class="admin-account-avatar-empty">-</span>`
          }
        </td>
      ...
      const rowAvatarEl = row.querySelector(".admin-account-avatar");
      if (rowAvatarEl) {
        rowAvatarEl.addEventListener(
          "error",
          () => {
            const currentSrc = String(rowAvatarEl.getAttribute("src") || "").trim();
            if (!currentSrc || currentSrc === MEMBER_AVATAR_FALLBACK) {
              return;
            }
            rowAvatarEl.setAttribute("src", MEMBER_AVATAR_FALLBACK);
          },
      ...
      adminAccountFormEl.addEventListener("submit", (event) => {
        ...
        if (existingIndex !== -1) {
          ...
          adminAccounts[existingIndex] = {
            ...
            discordAvatarUrl: discordUserId
              ? normalizeDiscordAvatarUrl(adminAccounts[existingIndex].discordAvatarUrl)
              : "",
            canAccessMembers,
            canAccessLiczniki,
            canAccessYoutube,
  ...
  function buildDiscordCdnAvatarUrl(discordUserId, avatarHash) {
    const id = normalizeDiscordUserId(discordUserId);
    const cleanHash = String(avatarHash || "").trim();
    if (!id || !cleanHash) {
      return "";
    }
    const extension = cleanHash.startsWith("a_") ? "gif" : "png";
    return `https://cdn.discordapp.com/avatars/${id}/${cleanHash}.${extension}?size=256`;
  }

  function buildDiscordDefaultAvatarUrl(discordUserId, discriminator = "") {
    const id = normalizeDiscordUserId(discordUserId);
    if (!id) {
      return "";
    }

    const cleanDiscriminator = String(discriminator || "").trim();
    if (cleanDiscriminator && cleanDiscriminator !== "0") {
      const discriminatorNumber = Number.parseInt(cleanDiscriminator, 10);
      if (Number.isFinite(discriminatorNumber)) {
        return `https://cdn.discordapp.com/embed/avatars/${Math.abs(discriminatorNumber) % 5}.png`;
      }
    }

    try {
      const avatarIndex = Number((BigInt(id) >> 22n) % 6n);
  ...
  function resolveDiscordSessionAvatarUrl(session, fallbackAccount = null) {
    const source = session && typeof session === "object" ? session : {};
    const fallback = fallbackAccount && typeof fallbackAccount === "object" ? fallbackAccount : {};
    const discordUserId = normalizeDiscordUserId(source.id || fallback.discordUserId);
    if (!discordUserId) {
      return "";
    }

    const fromSession = normalizeDiscordAvatarUrl(source.avatarUrl);
    if (fromSession) {
      return fromSession;
    }

    const avatarHash = String(source.avatarHash || source.avatar || "").trim();
    const fromHash = normalizeDiscordAvatarUrl(buildDiscordCdnAvatarUrl(discordUserId, avatarHash));
    if (fromHash) {
      return fromHash;
    }

    const fallbackStored = normalizeDiscordAvatarUrl(fallback.discordAvatarUrl);
    if (fallbackStored) {
      return fallbackStored;
    }

    return normalizeDiscordAvatarUrl(buildDiscordDefaultAvatarUrl(discordUserId, source.discriminator));
  }

  async function cacheDiscordAvatarInFolder(discordUserId, avatarUrl) {
    const cleanDiscordUserId = normalizeDiscordUserId(discordUserId);
    const cleanAvatarUrl = normalizeDiscordAvatarUrl(avatarUrl);
    if (!cleanDiscordUserId || !cleanAvatarUrl) {
      return cleanAvatarUrl;
    }
    if (cleanAvatarUrl.startsWith("/") || IS_FILE_PROTOCOL || typeof fetch !== "function") {
      return cleanAvatarUrl;
    }

    try {
      const response = await fetch(LOCAL_DISCORD_AVATAR_CACHE_ENDPOINT, {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
      ...
      const payload = await response.json();
      const localAvatarPath = normalizeDiscordAvatarUrl(
        payload && typeof payload === "object" ? payload.avatarPath || payload.avatarUrl || payload.publicUrl : ""
      );
      return localAvatarPath || cleanAvatarUrl;
    } catch (_error) {
      return cleanAvatarUrl;
    }
  }

  async function persistDiscordAvatarForAccount(session, preferredAccount = null) {
    const discordUserId = normalizeDiscordUserId(session && session.id);
    if (!discordUserId) {
      return "";
    }

    const accountFromState = getInlineAdminAccountsFallback().find(
      (entry) => normalizeDiscordUserId(entry && entry.discordUserId) === discordUserId
    );
    const targetAccount =
      preferredAccount && typeof preferredAccount === "object"
        ? preferredAccount
        : accountFromState && typeof accountFromState === "object"
          ? accountFromState
          : null;
    if (!targetAccount) {
      return "";
    ...
    const remoteAvatarUrl = resolveDiscordSessionAvatarUrl(session, targetAccount);
    if (!remoteAvatarUrl) {
      return "";
    }
    const cachedAvatarUrl = await cacheDiscordAvatarInFolder(discordUserId, remoteAvatarUrl);
    const nextAvatarUrl = normalizeDiscordAvatarUrl(cachedAvatarUrl || remoteAvatarUrl);
    if (!nextAvatarUrl) {
      return "";
    }

    const currentAvatarUrl = normalizeDiscordAvatarUrl(targetAccount.discordAvatarUrl);
    if (nextAvatarUrl === currentAvatarUrl) {
      return nextAvatarUrl;
    }

    targetAccount.discordAvatarUrl = nextAvatarUrl;
    saveAdminAccounts();
    renderAdminAccountsTable();
    return nextAvatarUrl;
  }

  async function ensureDiscordAvatarForActiveSession() {
    ...
    if (!normalizeDiscordUserId(currentAccount.discordUserId)) {
      return "";
    }
    const alreadyStored = normalizeDiscordAvatarUrl(currentAccount.discordAvatarUrl);
    if (alreadyStored) {
      return alreadyStored;
    }
    return persistDiscordAvatarForAccount(discordSession, currentAccount);
  }
  ...
  function getRootAdminAccount(discordName = "", discordAvatarUrl = "") {
    return {
      id: ROOT_ADMIN_ID,
      ...
      discordAvatarUrl: normalizeDiscordAvatarUrl(discordAvatarUrl),
      canAccessAdmin: true,
  ...
  function normalizeAdminAccountEntry(rawEntry, index = 0) {
    ...
    const discordAvatarUrl = normalizeDiscordAvatarUrl(
      rawEntry.discordAvatarUrl || rawEntry.discordAvatar || rawEntry.avatarUrl || rawEntry.avatar
    );
  ...
  function normalizeAdminAccounts(rawAccounts) {
    ...
    const rootDiscordAvatarUrl = rootIndex !== -1 ? normalizeDiscordAvatarUrl(normalized[rootIndex].discordAvatarUrl) : "";
    if (rootIndex === -1) {
      normalized.unshift(getRootAdminAccount(rootDiscordName, rootDiscordAvatarUrl));
    } else {
  ...
  function buildAdminAccountAuditSnapshot(account) {
    ...
    return {
      ...
      discordAvatarUrl: normalizeDiscordAvatarUrl(source.discordAvatarUrl),
  ...
  function hasSoonRouteAccess() {
    return hasValidAdminSession();
  }

  function syncSoonNavVisibility() {
    const canAccessSoon = hasSoonRouteAccess();
    setVisible(soonNavEl, canAccessSoon);
    if (!canAccessSoon && soonNavEl) {
      soonNavEl.classList.remove("is-active");
    }
  }
  ...
  function handleDiscordOAuthCallbackFallback() {
    ...
    Promise.resolve(window.StronaliveWebhook.completeDiscordAdminLogin(accounts))
      .then(async (result) => {
        ...
        try {
          const discordUserId = normalizeDiscordUserId(session.id || (result.linkedAccount && result.linkedAccount.discordUserId));
          const linkedAccount =
            discordUserId
              ? adminAccounts.find((entry) => normalizeDiscordUserId(entry && entry.discordUserId) === discordUserId) || null
              : null;
          persistedAvatarUrl = await persistDiscordAvatarForAccount(session, linkedAccount);
        } catch (_error) {
          persistedAvatarUrl = "";
        }
        ...
        sendAdminAuditEvent("admin_discord_login_success", {
          ...
          avatarStored: Boolean(String(persistedAvatarUrl || "").trim()),
  ...
  function resolveRouteByAuth(routeName) {
    ...
    if ((routeName === "soon" || routeName === "admin") && !hasAdminSession) {
      return "login";
    }
    if (routeName === "login" && hasAdminSession) {
      return "admin";
    }
    return routeName;
  }
  ...
  function applyRoute(routeName) {
    ...
    if (route === "admin") {
      ...
      void ensureDiscordAvatarForActiveSession();
    }
