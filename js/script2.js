...
const CHANNEL_SLUG = "takuu";
    ...
    const statsPanelEl = null;
    const karyTimerStatsGridEl = null;
    const karyCounterStatsGridEl = null;
    const wheelStatsSummaryEl = null;
    const wheelStatsSegmentBodyEl = null;
    const wheelStatsRecentListEl = null;
    const karyTimersGridEl = null;
    const karyCountersGridEl = null;
    const streamIntroActiveCountEl = null;
    const streamIntroLiveEl = null;
    const streamIntroActiveTextEl = null;
    const streamActiveKaryEl = null;
    const streamActiveKaryListEl = null;
    ...
    const KARY_ROUTE_PATH = HOME_ROUTE_PATH;
    ...
    const STATS_ROUTE_PATH = HOME_ROUTE_PATH;
    ...
    function updatePlaceholder(routeName) {
      ...
      routePlaceholderEl.style.display = shouldShow ? "" : "none";
    ...
    function applyRouteSurfaceVisibility(routeName, adminAuthenticated = isAdminAuthenticated) {
      ...
      const isAdminRoute = routeName === "admin";
      const showLogin = routeName === "login" || (isAdminRoute && !adminAuthenticated);
      const showAdmin = isAdminRoute && adminAuthenticated;
    ...
    // Apply the correct route surface immediately on first load, before the rest
    // of the runtime finishes binding optional features.
    applyRouteSurfaceVisibility(getRouteFromPath(`${window.location.pathname}${window.location.search}${window.location.hash}`));
    ...
    function renderKaryStats() {
      ...
      syncKaryStatsRecordsFromCurrentState({ render: false });
    ...
    function applyWheelWinnerSync(rawPayload, source = "sync") {
      ...
      if (payload.minutes > 0) {
        ...
        candidateTimerKeys.forEach((timerKey) => {
          addTimerTimeFromExternal(timerKey, payload.minutes, "minutes", {
            silentStatus: true,
            emitWebhook: false,
    ...
    function canAccountAccessAnyAdminArea(account) {
      ...
      return Boolean(account.canAccessAdmin || account.canAccessBindings);
    }
    ...
    function setActiveAdminTab(tabName) {
      ...
      const bindingsAccess = hasBindingsAccess();
      const requestedTab = tabName === "accounts" || tabName === "bindings" || tabName === "members"
        ? tabName
        : "members";
    ...
    async function handleDiscordOAuthCallback() {
      ...
      if (!isAdminAuthenticated) {
        setAdminStatus("Konto Discord zapisane, ale nie ma permisji do Panelu Admina ani Powiązań.", "error");
    ...
    async function startDiscordLoginFlow() {
      if (!window.TakuuWebhook || typeof window.TakuuWebhook.startDiscordLogin !== "function") {
        setDiscordStatus("Brak modulu discord.js dla logowania Discord.", "error");
        return;
      }
    ...
    function applyView(routeName) {
      ...
      const showHome = routeName === "home";
      const isAdminRoute = routeName === "admin";
      const showLogin = routeName === "login" || (isAdminRoute && !isAdminAuthenticated);
      const showAdmin = isAdminRoute && isAdminAuthenticated;
      if (!showClips) {
        closeClipViewer();
      }
      ...
      applyRouteSurfaceVisibility(routeName, isAdminAuthenticated);
    ...
    if (adminDiscordLoginBtnEl) {
      if (window.TakuuWebhook && typeof window.TakuuWebhook.isDiscordLoginAvailable === "function") {
        ...
        adminDiscordLoginBtnEl.disabled = true;
        setDiscordStatus("Brak discord.js lub konfiguracji Discord.", "error");
      }
    ...
    handleDiscordOAuthCallback().finally(() => {
      ...
      const initialPathRoute = getRouteFromPath(window.location.pathname);
...
/* INLINE_FALLBACK_MOVED_FROM_INDEX */
    (function () {
      ...
      function detectRoute() {
        ...
        if (/\/kary\/?$/.test(path) || view === "kary") return "home";
        ...
        if (/\/(stats|statystyki)\/?$/.test(path) || view === "stats" || view === "statystyki") return "home";
      ...
      function tryInlineLocalLoginFallback(login, password) {
        ...
        for (var i = 0; i < accounts.length; i += 1) {
          ...
          if (!(account && (account.canAccessAdmin || account.canAccessBindings || account.isRoot))) continue;
      ...
      function bindInlineLoginFallback(route) {
        ...
        if (discordBtn) {
          if (!window.TakuuWebhook || typeof window.TakuuWebhook.startDiscordLogin !== "function") {
            ...
            setInlineLoginStatus(discordStatus, "Brak discord.js lub konfiguracji Discord.", "error");
          } else {
            if (typeof window.TakuuWebhook.isDiscordLoginAvailable === "function") {
      ...
      document.addEventListener("DOMContentLoaded", function () {
        ...
        applyRouteSurfaceVisibility(route);
